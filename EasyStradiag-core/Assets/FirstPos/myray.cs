...
public class myray : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hit, 1000f, 1 << 5))//如果触碰到了物体，第五层UI层上的物体
        {
            ...
            if (hit.collider.gameObject.CompareTag("Mytagg"))//需要指定button的layer层以及Tag，指定Tag方便射线照射物体时可以更省内存。
            {
                ...
                {
                    obj.transform.Translate(Vector3.forward * Time.deltaTime);//forward 鱼的z减小，越接近相机-10
                }
            }
        }
        else
